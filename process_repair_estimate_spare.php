...
if ($_SERVER['REQUEST_METHOD'] !== 'POST' || empty($_POST['vehicle_id'])) {
    ...
    header("Location: view_estimates_spare.php");
    exit();
}
...
$result = Database::search("SELECT COUNT(*) as count FROM estimates_spareparts WHERE DATE(created_at) = CURDATE()");
$count = intval($result->fetch_assoc()['count']) + 1;
$estimate_number = sprintf("EST%s%03d", date('Ymd'), $count);

// Insert estimate
$estimate_query = "INSERT INTO estimates_spareparts (vehicle_id, estimate_number, total_amount, estimate_date, notes) 
...
foreach ($estimate_items as $item) {
    ...
    Database::iud("INSERT INTO estimate_items_spareparts (estimate_id, price, description) 
                  VALUES ('$estimate_id', '$price', '$description')");
}

$_SESSION['success'] = 'Estimate #' . $estimate_number . ' created successfully';
header("Location: view_repair_estimate_spare.php?id=" . $estimate_id);
exit();
?>