...
if (!$id) {
    ...
    header('Location: view_repair_estimate_sup.php');
    exit();
}
...
if ($result->num_rows === 0) {
    ...
    header('Location: supplementary_estimates.php');
    exit();
}

$estimate = $result->fetch_assoc();

$items_query = "SELECT * FROM estimate_items_supplimentary WHERE estimate_id = '$id' ORDER BY category";
...
<div class="container content">
    ...
    <div class="row mb-3">
        <div class="col-md-6">
            <h2>View Supplementary Estimate #<?php echo htmlspecialchars($estimate['estimate_number']); ?></h2>
        </div>
...
<script>
    function printEstimate(id) {
        ...
        form.method = 'POST';
        form.action = 'print_repair_estimate_sup.php?id=' + encodeURIComponent(id);
        form.target = '_blank';
